...
(similar to an n-gram model). This implies some practical differences, for example
    ...
    (c) Modeling words may lead to a simpler model compared to character-level models. However, 
    it may struggle with handling rare words or unseen combinations.

    (d) Character-level models may require more data to learn effective representations due to 
    their inability to represent semantic meaning as good as word-level models. They also may 
    take longer to train due to the increased input size and complexity.

    (e) Character-level models can capture language variations, dialects, and even stylistic 
...
serve several purposes like: 
    ...
    -ensuring consistent encoding and compatibility with existing libraries (in this case torch)
...
4.1. The --default_train flag, triggers a standard training process with default hyperparameters of 
an LSTM (Long Short-Term Memory) model for character-level language generation. The defined parameters
will be used to initialize the model with the necesarry information, i.e. input dimension, hidden 
dimension, number of layers and output dimensions. An Adam optimizer is also employed.

The script then enters a training loop that runs for a predefined number of epochs (n_epochs). In each epoch, 
the train function is called with the LSTM model, Adam optimizer, and a random training set. Training loss 
is computed and accumulated. If the current epoch is a multiple of print_every, training progress is printed, 
including time elapsed, epoch number, percentage completion, and the current loss. A sample text is also 
generated using the generate function and printed. If the current epoch is a multiple of plot_every, the 
average loss over the plotting interval is recorded. This information could be used to generate a plot of the 
loss and analyze the model.

4.2. See "./results/custom.log". My approach was to test the following hyperparameters and compare their results between each other and with
the default settings:
    ...
    (b) {'hidden_size': 64, 'n_layers': 1, 'lr': 0.01, 'temperature': 0.5}
    Moderate learning rate, moderate hidden size, moderate temperature and a single layer.
    (c) {'hidden_size': 256, 'n_layers': 3, 'lr': 0.002, 'temperature': 0.9}
    ...
    hyperparemeters' combination. This way I could identify how sensitive the model is to different hyperparameters 
    and which parameters have a significant impact on performance. Here are some observations of each test:
    ...
    (b) Balanced convergence speed and stability. Moderately diverse and focused generated text. Captured basic
    patterns but struggled with complexity.
    ...
    (d) Good but slower convergence, but risk of overfitting. Good capacity to capture complex patterns. Increased randomness 
    in generated text. Improved ability to capture hierarchical dependencies.
    ...
    (e) A high number of epochs helps to a better convergence, but can end up in overfitting. This is the most difficult
    part to choose, in my opinion.

4.3. See "./results/output.jpg"

4.4. See "./results/diff.log". When increasing the temperature value we observe changes in the diversity and randomness of the 
generated text. With low temperatures, we got more deterministic and focused text. With high temperatures,
we got more diverse but less focused output. Intuitevily, I did not understand why a bigger temperature 
would end up in more randomized texts. However, according to Hinton et al (2015), this hyperparameter is
used to scale the logits before applying softmax. When the temperature is 1, we compute the softmax 
directly on the logits. When the temperature is smaller, the softmax ends up being higher needing less input
to activate the layer, but also making it less likely to sample from candidates with smaller numbers. This
would explain why a higher temperature ends up in a more randomized answer.
    ...
    must exercise caution and critical judgment. In the best case scenario, there is an open and always available
    communication between the three instances.
    ...
    model training. In best case-scenario, also include minorities or unrepresented groups in the testing phase.
